...
srand(time(0));
for(int i=0;i<size;i++) scores[i] = ran() % 101;
printf("입력완료\n");
...
case 2:
  ...
  deleteStudent(scores, size, sudentNumber);
  break;
case 3:
...
void deleteStudent(int* scores, size, int studentNumber){  
  if(student number < 1 || studentNumber > size){
    printf("학생 번호 오류\n");
  }
...
double calculateAverge(int scores[], int size){}
int getstudentScore(int scores[], int size, int studentNumber){}
void printScore(int scores[], int size, double average, int option){
